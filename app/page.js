...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-8">
      ...
      {/* Tambahan: Laporan dan Grafik Analisis */}
      <Dialog>
        <DialogContent className="sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>Laporan Analisis Tugas</DialogTitle>
          </DialogHeader>
          <Tabs defaultValue="daily">
            ...
            <TabsContent value="daily" className="space-y-4">
              <Card>
                ...
                <CardContent>
                  <div className="h-80">
                    {/* Disini bisa ditambahkan komponen chart untuk data harian */}
                    <ReportChart 
                      data={generateDailyReportData(tasks)} 
                      type="daily" 
                    />
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Ringkasan Status Harian</CardTitle>
                </CardHeader>
                <CardContent>
                  <TaskStatusSummary 
                    data={generateDailyStatusData(tasks)} 
                    period="Hari Ini"
                  />
                </CardContent>
              </Card>
            ...
            <TabsContent value="weekly" className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Tugas Mingguan</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="h-80">
                    <ReportChart 
                      data={generateWeeklyReportData(tasks)} 
                      type="weekly" 
                    />
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Ringkasan Status Mingguan</CardTitle>
                </CardHeader>
                <CardContent>
                  <TaskStatusSummary 
                    data={generateWeeklyStatusData(tasks)} 
                    period="Minggu Ini"
                  />
                </CardContent>
              </Card>
            ...
            <TabsContent value="monthly" className="space-y-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Tugas Bulanan</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="h-80">
                    <ReportChart 
                      data={generateMonthlyReportData(tasks)} 
                      type="monthly" 
                    />
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">Ringkasan Status Bulanan</CardTitle>
                </CardHeader>
                <CardContent>
                  <TaskStatusSummary 
                    data={generateMonthlyStatusData(tasks)} 
                    period="Bulan Ini"
                  />
                </CardContent>
              </Card>
...
// Komponen Laporan Chart
const ReportChart = ({ data, type }) => {
  if (!data || data.length === 0) {
    return <div className="flex items-center justify-center h-full text-gray-500">Tidak ada data tersedia</div>;
  }
  
  return (
    <div className="w-full h-full">
      {type === 'daily' && (
        <BarChart width={700} height={300} data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
      ...
      {type === 'weekly' && (
        <LineChart width={700} height={300} data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="day" />
          <YAxis />
          <Tooltip formatter={(value) => [`${value} tugas`, 'Jumlah']} />
          ...
          <Line type="monotone" dataKey="In Progress" stroke="#60a5fa" strokeWidth={2} />
      ...
      {type === 'monthly' && (
        <AreaChart width={700} height={300} data={data} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="date" />
          <YAxis />
          <Tooltip formatter={(value) => [`${value} tugas`, 'Jumlah']} />
          <Legend />
          <Area type="monotone" dataKey="Completed" stackId="1" stroke="#4ade80" fill="#4ade80" fillOpacity={0.6} />
          <Area type="monotone" dataKey="In Progress" stackId="1" stroke="#60a5fa" fill="#60a5fa" fillOpacity={0.6} />
          <Area type="monotone" dataKey="Pending" stackId="1" stroke="#fbbf24" fill="#fbbf24" fillOpacity={0.6} />
          <Area type="monotone" dataKey="Cancelled" stackId="1" stroke="#f87171" fill="#f87171" fillOpacity={0.6} />
        </AreaChart>
      )}
    </div>
  );
...
// Komponen Ringkasan Status
const TaskStatusSummary = ({ data, period }) => {
  if (!data) {
    return <div className="text-gray-500">Tidak ada data tersedia</div>;
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div className="p-4 bg-green-50 rounded-lg border border-green-100">
        ...
        <div className="text-2xl font-bold text-green-700">{data.Completed || 0}</div>
        <div className="text-sm text-green-600">{period}</div>
      </div>
      
      <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
        <div className="text-blue-800 font-medium">Sedang Dikerjakan</div>
        <div className="text-2xl font-bold text-blue-700">{data["In Progress"] || 0}</div>
        <div className="text-sm text-blue-600">{period}</div>
      </div>
      
      <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-100">
        <div className="text-yellow-800 font-medium">Tertunda</div>
        <div className="text-2xl font-bold text-yellow-700">{data.Pending || 0}</div>
        <div className="text-sm text-yellow-600">{period}</div>
      </div>
      
      <div className="p-4 bg-red-50 rounded-lg border border-red-100">
        <div className="text-red-800 font-medium">Dibatalkan</div>
        <div className="text-2xl font-bold text-red-700">{data.Cancelled || 0}</div>
        <div className="text-sm text-red-600">{period}</div>
      </div>
    </div>
  );
};

// Fungsi helper untuk membuat data laporan
const generateDailyReportData = (tasks) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  
  // Kelompokkan per jam dalam sehari
  ...
  return hours.map(hour => {
    ...
    // Filter tugas per status dalam rentang jam tersebut
    ...
    const completed = hourTasks.filter(task => task.status === "Completed").length;
    const inProgress = hourTasks.filter(task => task.status === "In Progress").length;
    const pending = hourTasks.filter(task => task.status === "Pending").length;
    const cancelled = hourTasks.filter(task => task.status === "Cancelled").length;
...
const generateWeeklyReportData = (tasks) => {
  const today = new Date();
  const weekStart = startOfWeek(today, { weekStartsOn: 1 });
  
  // Membuat array untuk 7 hari dalam seminggu
  ...
  return days.map(day => {
    ...
    // Filter tugas per status dalam rentang hari tersebut
...
const generateMonthlyReportData = (tasks) => {
  const today = new Date();
  const monthStart = startOfMonth(today);
  const monthEnd = endOfMonth(today);
  const daysInMonth = eachDayOfInterval({ start: monthStart, end: monthEnd });
  
  // Kelompokkan per tiga hari untuk mengurangi jumlah data point
  const groups = [];
  for (let i = 0; i < daysInMonth.length; i += 3) {
    const groupEnd = Math.min(i + 2, daysInMonth.length - 1);
    groups.push(daysInMonth.slice(i, groupEnd + 1));
  }
  
  return groups.map(dayGroup => {
    const groupStart = dayGroup[0];
    const groupEnd = dayGroup[dayGroup.length - 1];
    
    const dayStart = new Date(groupStart);
    dayStart.setHours(0, 0, 0, 0);
    
    const dayEnd = new Date(groupEnd);
    dayEnd.setHours(23, 59, 59, 999);
    ...
    return {
      date: `${format(groupStart, "d", { locale: id })}-${format(groupEnd, "d", { locale: id })}`,
      Completed: completed,
      "In Progress": inProgress,
      Pending: pending,
      Cancelled: cancelled,
...
// Fungsi untuk menghasilkan data ringkasan status
const generateDailyStatusData = (tasks) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  return {
    ...
    Cancelled: todayTasks.filter(task => task.status === "Cancelled").length
  };
};

const generateWeeklyStatusData = (tasks) => {
  const today = new Date();
  const weekStart = startOfWeek(today, { weekStartsOn: 1 });
  const weekEnd = endOfWeek(today, { weekStartsOn: 1 });
  
  // Filter tugas untuk minggu ini
  ...
  return {
    ...
    "In Progress": weekTasks.filter(task => task.status === "In Progress").length,
    Pending: weekTasks.filter(task => task.status === "Pending").length,
    Cancelled: weekTasks.filter(task => task.status === "Cancelled").length
  };
};

const generateMonthlyStatusData = (tasks) => {
  const today = new Date();
  const monthStart = startOfMonth(today);
  const monthEnd = endOfMonth(today);
  
  // Filter tugas untuk bulan ini
  ...
  return {
    ...
    "In Progress": monthTasks.filter(task => task.status === "In Progress").length,
    Pending: monthTasks.filter(task => task.status === "Pending").length,
    Cancelled: monthTasks.filter(task => task.status === "Cancelled").length
  };
};